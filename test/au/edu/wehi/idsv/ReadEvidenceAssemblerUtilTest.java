...
public class ReadEvidenceAssemblerUtilTest extends TestHelper {
	...
	public void anchor_should_use_reference_base_not_assembly_base() {
		String alt = ReadEvidenceAssemblerUtil.breakendBuilder(getContext(), "test", 0, 1, BreakendDirection.Forward, B("A"), B("CC"), 1, 1, 0).make().getAlternateAllele(0).getDisplayString();
		assertEquals('A', alt.charAt(0));
	}
	@Test
	public void should_assembly_base_qualities() {
		VariantContextDirectedBreakpoint dba = ReadEvidenceAssemblerUtil.breakendBuilder(getContext(), "test", 0, 1, BreakendDirection.Forward, B("TTT"), new byte[] { 1,2,3 }, B("ATTT"), new byte[] { 1,2,3,4}, 1, 1, 0).make();
		assertArrayEquals(new byte[] {1, 2, 3}, dba.getBreakpointQuality());
	}
	@Test
	public void should_set_assembly_evidence() {
		VariantContextDirectedBreakpoint dba = ReadEvidenceAssemblerUtil.breakendBuilder(getContext(), "test", 0, 1, BreakendDirection.Forward, B("TTT"), new byte[] { 1,2,3 }, B("ATTT"), new byte[] { 1,2,3,4}, 1, 2, 3).make();
