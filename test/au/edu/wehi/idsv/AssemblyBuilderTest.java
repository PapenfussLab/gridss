...
public class AssemblyBuilderTest extends TestHelper {
	...
	public void should_filter_fully_reference_assemblies() {
		AssemblyBuilder ab = new AssemblyBuilder(getContext())
			.assemblyBases(B("AA"))
			.anchorLength(2)
			.referenceAnchor(0, 1)
			.direction(BWD);
	...
	public void should_set_assembly_evidence() {
		VariantContextDirectedBreakpoint dba = new AssemblyBuilder(getContext())
	...
	public void should_set_breakend_sequence() {
		assertEquals("GT", new AssemblyBuilder(getContext())
			.assemblyBases(B("GTAC"))
			.referenceAnchor(0, 1)
			.direction(BWD)
			.anchorLength(2)
			.makeVariant().getBreakpointSequenceString());
		assertEquals("AC", new AssemblyBuilder(getContext())
			.assemblyBases(B("GTAC"))
			.referenceAnchor(0, 10)
			.direction(FWD)
			.anchorLength(2)
	...
	public void should_breakend_to_reference_anchor() {
		...
		BreakendSummary bs;
		dba = new AssemblyBuilder(getContext())
			.assemblyBases(B("AAAAAA"))
	...
	public void should_set_breakend_to_mate_anchor_interval() {
		...
		assertEquals(BWD, bs.direction);
		assertEquals(1, bs.referenceIndex);
		assertEquals(1000 - getContext().getMetrics().getMaxFragmentSize(), bs.start);
		...
		assertEquals(1000 + getContext().getMetrics().getMaxFragmentSize(), bs.end);
	}
	@Test
