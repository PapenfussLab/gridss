...
public class AnnotateBreakends extends CommandLineProgram {
	...
	public String USAGE = getStandardUsagePreamble()
			+ "Annotates breakend calls with supporting evidence "
			+ PROGRAM_VERSION;
	@Option(doc = "Input BAM file.", optional = false, shortName = StandardOptionDefinitions.INPUT_SHORT_NAME)
	public File INPUT;
	@Option(doc = "Breakpoint calls in VCF format", optional = true, shortName = StandardOptionDefinitions.OUTPUT_SHORT_NAME)
	public File OUTPUT;
	@Option(doc = "Reference used for alignment", optional = false, shortName = StandardOptionDefinitions.REFERENCE_SHORT_NAME)
	public File REFERENCE;
	@Option(doc = "Picard metrics file generated by ExtractEvidence", optional = true)
	...
	protected int doWork() {
		try {
			...
			final ProcessingContext processContext = getContext(REFERENCE,
					INPUT);

			final SamReader inputReader = getSamReaderFactory().open(INPUT);
			final CloseableIterator<SAMRecord> inputIterator = inputReader
					.iterator();
			final SequentialReferenceCoverageLookup referenceLookup = new SequentialReferenceCoverageLookup(
					3 * processContext.getMetrics().getMaxFragmentSize(),
					inputIterator);
			...
			final SequentialBreakendAnnotator annotator = new SequentialBreakendAnnotator(
					processContext, referenceLookup,
					Iterators.peekingIterator(evidenceIt));
			...
			while (toAnnotateIt.hasNext()) {
				VariantContextDirectedBreakpoint annotatedVariant = annotator
						.annotate(toAnnotateIt.next());
	...
	private Iterator<DirectedEvidence> getAllEvidence(
		...
		if (FileNamingConvention.getSVBam(INPUT).exists()) {
			...
			final DirectedEvidenceFileIterator dei = new DirectedEvidenceFileIterator(
					...
					ensureFileExists(FileNamingConvention
							.getRealignmentBam(INPUT)),
			...
			for (SAMSequenceRecord seq : processContext.getDictionary()
					.getSequences()) {
				String chr = seq.getSequenceName();
				itList.add(new DirectedEvidenceFileIterator(processContext,
						getSamReaderFactory(),
						ensureFileExists(FileNamingConvention.getSVBamForChr(
								INPUT, chr)),
						ensureFileExists(FileNamingConvention.getMateBamForChr(
								INPUT, chr)),
						ensureFileExists(FileNamingConvention
								.getRealignmentBamForChr(INPUT, chr)),
						ensureFileExists(FileNamingConvention
								.getBreakendVcfForChr(INPUT, chr))));
			}
			return Iterators.concat(itList.iterator());
		}
	}

	private Iterator<VariantContextDirectedBreakpoint> getCallsToAnnotate(
			final ProcessingContext processContext) throws IOException {
		// open VCF
		// (assert sorted?)
		if (FileNamingConvention.getRawCallVcf(INPUT).exists()) {
			...
			List<SAMSequenceRecord> seqList = processContext.getDictionary()
					.getSequences();
			for (int i = 0; i < seqList.size(); i++) {
				for (int j = i; j < seqList.size(); j++) { // start at i so we
															// don't repeat
			...
			return Iterators.mergeSorted(itList,
					IdsvVariantContext.ByStartStopReferenceOrder);
		}
	}
