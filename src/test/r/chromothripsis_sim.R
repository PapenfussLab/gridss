...
ggplot(dtroc[dtroc$CX_READ_FRAGMENT_LENGTH==400 & dtroc$CX_READ_LENGTH==100,]) +
...
# assembly rate
ggplot(calls, aes(x=QUAL-ASQ-RASQ, fill=paste(pmin(AS, RAS), "/", pmax(AS, RAS), "assemblies"))) +
  geom_histogram() +
  scale_x_log10(limits=c(20, max(calls$QUAL-calls$ASQ-calls$RASQ))) +
  labs(title="Assembly rate", x="Evidence quality")
ggsave("assembly_rate_qual.png", width=10, height=7.5)
ggplot(calls) +
  aes(x=SC+RSC+RP+BRP+BSC, fill=paste(AS, "assemblies")) +
  geom_histogram() +
  scale_x_log10() +
  labs(title="Assembly rate", x="Reads supporting variant")
