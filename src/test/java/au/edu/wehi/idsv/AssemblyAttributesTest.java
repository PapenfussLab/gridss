...
public class AssemblyAttributesTest extends TestHelper {
	...
	public void setEvidenceIDs_should_set_same_value_regardless_of_collection_ordering() {
		...
		SAMRecord ass1 = AssemblyFactory.createUnanchoredBreakend(getContext(), AES(), new BreakendSummary(0, FWD, 1, 1, 2), ImmutableList.of(e1, e2), B("GTAC"), new byte[] {1,2,3,4}, new int[] {0, 0});
		SAMRecord ass2 = AssemblyFactory.createUnanchoredBreakend(getContext(), AES(), new BreakendSummary(0, FWD, 1, 1, 2), ImmutableList.of(e2, e1), B("GTAC"), new byte[] {1,2,3,4}, new int[] {0, 0});
