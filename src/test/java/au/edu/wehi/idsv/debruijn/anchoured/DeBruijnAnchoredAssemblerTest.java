...
public class DeBruijnAnchoredAssemblerTest extends TestHelper {
	public List<AssemblyEvidence> go(int k, DirectedEvidence... evidence) {
		...
		for (DirectedEvidence e : evidence) {
			...
			if (it != null) {
				for (AssemblyEvidence ass : it) {
					list.add(ass);
				}
			}
		}
		Iterable<AssemblyEvidence> it = assembler.endOfEvidence();
		if (it != null) {
			for (AssemblyEvidence ass : it) {
				list.add(ass);
			}
	...
	public void should_call_multiple_soft_clips() {
		...
		assertEquals("ACGTGA", S(r.get(0).getBreakendSequence()));
		assertEquals("A", S(r.get(0).getAssemblyAnchorSequence()));
		assertEquals(2, r.get(0).getAssemblySupportCountSoftClip(EvidenceSubset.ALL));
