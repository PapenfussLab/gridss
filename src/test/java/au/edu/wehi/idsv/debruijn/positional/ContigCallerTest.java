...
public abstract class ContigCallerTest extends TestHelper {
	...
	public void should_start_path_with_no_predecessor() {
		...
		String result = S(caller.bestContig(), k);
	...
	public void should_not_start_in_reference_kmer() {
		...
		ArrayDeque<KmerPathSubnode> result = getCaller(input, 100000).bestContig();
		assertNull(result);
	}
	@Test
	...
	public void should_assemble_overlapping_sc_rp() {
		...
		String result = S(getCaller(in, 1000).bestContig(), 4);
	...
	public void should_preference_anchored_paths() {
		...
		String result = S(getCaller(Lists.newArrayList(asKPN(4, 100, spanf, spanb, scf, scb, e)), 100).bestContig(), 4);
