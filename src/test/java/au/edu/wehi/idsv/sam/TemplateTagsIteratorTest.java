...
public class TemplateTagsIteratorTest extends TestHelper {
    ...
    public void should_jointly_process_tags() {
        ...
        TemplateTagsIterator tti = new TemplateTagsIterator(TemplateTagsIterator.withGrouping(rec.iterator()), true, true, true, true, true, true, Sets.newHashSet(
                SAMTag.NM.name(),
                SAMTag.SA.name(),
                SAMTag.R2.name(),
                SAMTag.MC.name(),
                SAMTag.MQ.name()));
        List<SAMRecord> out = Lists.newArrayList(new UngroupingIterator<>(tti));
