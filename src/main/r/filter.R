...
df$A_SC[duplicated(df$variantid)] = df$A_SC[duplicated(df$variantid)] / 2 # Temp hack to fix counts
...
som <- df[
  ...
  df$RCNormal >= 8 & # min coverage in the normal supporting the reference allele
...
hdr <- VariantAnnotation:::.makeVcfHeader(somvcf)
data <- VariantAnnotation:::.makeVcfMatrix(somvcf)
sessionInfo()
