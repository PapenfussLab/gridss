...
public class DeBruijnPathGraph<T extends DeBruijnNodeBase, PN extends PathNode<T>> {
	...
	protected void addNode(PN node) {
		...
		pathList.add(node);
		pathNext.add(Lists.<PN>newArrayListWithExpectedSize(4));
		pathPrev.add(Lists.<PN>newArrayListWithExpectedSize(4));
		pathCount++;
	}
	/**
	...
	public int collapseLeaves(int maxDifference) throws AlgorithmRuntimeSafetyLimitExceededException {
		...
		do {
			...
			PriorityQueue<PN> ordering = new PriorityQueue<PN>(pathCount, ByPathTotalWeightDesc.reverse());
	...
	public boolean mergePaths(Iterable<PN> pathA, Iterable<PN> pathB) {
		...
		if (problematicNode != null) {
			if (log != null && inconsistentMergePathRemainingCalls > 0) {
				...
				log.debug(String.format("Near %d: found similar but inconsistent paths \"%s\" and \"%s\". Both contain \"%s\"",
					(long)getGraph().getKmer(pathA.iterator().next().getFirst()).getExpectedPosition(),
					new String(getGraph().getBaseCalls(Lists.newArrayList(PathNode.kmerIterator(pathA)))),
					new String(getGraph().getBaseCalls(Lists.newArrayList(PathNode.kmerIterator(pathB)))),
	...
	public String toString() {
		...
		for (PN x : getPaths()) {
			...
			sb.append(" <-{");
			for (PN s : prevPath(x)) sb.append(String.format("%d,", s.getNodeId()));
			sb.append("} ->{");
			for (PN s : nextPath(x)) sb.append(String.format("%d,", s.getNodeId()));
			sb.append("}\n");
		}
		return sb.toString();
	}
