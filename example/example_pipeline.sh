...
java $JVM_ARGS -cp $GRIDSS_JAR gridss.ComputeSamTags \
	...
	AS=true \
	| \
tee $INPUT_WORKING_PREFIX.unsorted.bam | \
# THRESHOLD_COVERAGE=gridss.properties/maxCoverage 
java -cp $GRIDSS_JAR gridss.analysis.CollectGridssMetrics \
	REFERENCE_SEQUENCE=$REFERENCE \
	I=/dev/stdin \
	O=$INPUT_WORKING_PREFIX \
	THRESHOLD_COVERAGE=10000
java $JVM_ARGS -cp $GRIDSS_JAR gridss.SoftClipsToSplitReads \
	REFERENCE_SEQUENCE=$REFERENCE \
	WORKING_DIR=$INPUT_WORKING_DIR \
	TMP_DIR=$INPUT_WORKING_DIR \
	I=$INPUT_WORKING_PREFIX.unsorted.bam \
	O=$INPUT_WORKING_PREFIX.withsplitreads.bam \
	WORKER_THREADS=$(nproc) \
	COMPRESSION_LEVEL=0 \
	
	& \
rm $INPUT_WORKING_PREFIX.unsorted.bam & \
samtools sort -O BAM -T $INPUT_WORKING_PREFIX.sorting $INPUT_WORKING_PREFIX.withsplitreads.bam > $INPUT & \
rm $INPUT_WORKING_PREFIX.withsplitreads.bam

# TODO: piped SoftClipsToSplitReads (can ignore sort order)
